...
- [x] 1. Harness: deterministic core (event queue + RNG)
  - [x] 1.1 Scaffold the `tools/sim-harness` library crate
  ...
  - [x] 1.2 Implement `Rng` in `tools/sim-harness/src/rng.rs`
  ...
  - [x] 1.3 Implement `EventQueue<E>`, `Scheduled<E>`, and `SimCtx<'a, E>` in `tools/sim-harness/src/event.rs`
...
- [x] 2. Harness: invariants, faults, reporting, CLI
  - [x] 2.1 Implement the invariant registry in `tools/sim-harness/src/invariant.rs`
  ...
  - [x] 2.2 Implement the fault framework in `tools/sim-harness/src/fault.rs`
  ...
  - [x] 2.3 Implement `SignalCounters` and `Report` in `tools/sim-harness/src/report.rs`
  ...
  - [x] 2.4 Implement the `StressModel` trait and `StressRunner` in `tools/sim-harness/src/lib.rs` (or `stress.rs`)
  ...
  - [x] 2.5 Implement the bounded-exhaustive enumerator in `tools/sim-harness/src/enumerate.rs`
  ...
  - [x] 2.6 Implement CLI scaffolding in `tools/sim-harness/src/cli.rs`
...
- [x] 3. Checkpoint — harness foundation
  - Run `cargo +nightly fmt --all --check`.
  - Run `cargo lint`.
  - Run `cargo test -p sim-harness`.
  - Ensure all tests pass; ask the user if questions arise.

- [x] 4. Harness reusability validation
  - [x] 4.1 Add a trivial second `StressModel` + `ExhaustiveModel` in `tools/sim-harness` tests
...
- [x] 5. Broker model: domain types and pure state
  - [x] 5.1 Scaffold the `tools/broker-sim` binary crate
  ...
  - [x] 5.2 Define the re-modelled domain identifiers and keys in `tools/broker-sim/src/model.rs`
  ...
  - [x] 5.3 Define `BrokerState` and `AuthoritativePendingState` in `tools/broker-sim/src/model.rs`
...
- [x] 6. Broker model: events and delivery semantics
  - [x] 6.1 Define `BrokerEvent` in `tools/broker-sim/src/events.rs`
...
- [x] 7. Broker model: invariants (S1–S7, L1–L4)
  - [x] 7.1 Implement the safety invariants in `tools/broker-sim/src/invariants.rs`
  ...
  - [x] 7.2 Implement the liveness invariants in `tools/broker-sim/src/invariants.rs`
...
- [x] 8. Verification modes wiring
  - [x] 8.1 Implement the bounded-exhaustive `BrokerActionModel` in `tools/broker-sim/src/exhaustive.rs`
...
- [x] 9. Faults and injectable bug
  - [x] 9.1 Register the required fault set in `tools/broker-sim/src/faults.rs`
  ...
  - [x] 9.2 Document the fault→invariant map in `tools/broker-sim/README.md`
  ...
  - [x] 9.3 Implement the injectable bug in `tools/broker-sim/src/bug.rs`
