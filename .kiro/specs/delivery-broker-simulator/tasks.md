# Implementation Plan: Delivery Broker Simulator and Shared Simulation Harness

## Overview

Build, in dependency order, the reusable simulation harness (`tools/sim-harness`, library) and the
delivery-broker simulator (`tools/broker-sim`, binary) on top of it, strictly per `design.md`.
The harness generalises the proven `tools/placement-sim` mechanics (xorshift64 RNG, `BinaryHeap`
...
it without harness changes. The broker model is a pure deterministic re-model of
...
- [ ] 1. Harness: deterministic core (event queue + RNG)
  - [ ] 1.1 Scaffold the `tools/sim-harness` library crate
    - Create `tools/sim-harness/Cargo.toml` (`publish = false`, `edition = "2024"`, no `tokio`, no `proptest` in non-dev deps, no Tokeira-crate dependency) and `src/lib.rs` exposing the `rng`, `event`, `invariant`, `fault`, `enumerate`, `report`, `cli` modules.
    - _Requirements: 7.1, 7.2, 34.1, 34.2, 34.3, 34.5_
  - [ ] 1.2 Implement `Rng` in `tools/sim-harness/src/rng.rs`
  ...
  - [ ] 1.3 Implement `EventQueue<E>`, `Scheduled<E>`, and `SimCtx<'a, E>` in `tools/sim-harness/src/event.rs`
  ...
  - [ ] 1.4 Unit tests for RNG determinism and event ordering
    - Same seed → identical `next_u64` stream; events with equal `at_ms` drain in `seq` order; `schedule` honors `now_ms + delay`.
    - _Requirements: 1.1, 1.2, 1.4_

- [ ] 2. Harness: invariants, faults, reporting, CLI
  - [ ] 2.1 Implement the invariant registry in `tools/sim-harness/src/invariant.rs`
  ...
  - [ ] 2.2 Implement the fault framework in `tools/sim-harness/src/fault.rs`
  ...
  - [ ] 2.3 Implement `SignalCounters` and `Report` in `tools/sim-harness/src/report.rs`
  ...
  - [ ] 2.4 Implement the `StressModel` trait and `StressRunner` in `tools/sim-harness/src/lib.rs` (or `stress.rs`)
  ...
  - [ ] 2.5 Implement the bounded-exhaustive enumerator in `tools/sim-harness/src/enumerate.rs`
  ...
  - [ ] 2.6 Implement CLI scaffolding in `tools/sim-harness/src/cli.rs`
  ...
  - [ ] 2.7 Unit tests for registry, enumerator, report, and CLI
...
- [ ] 3. Checkpoint — harness foundation
  - Run `cargo +nightly fmt --all --check`.
  - Run `cargo lint`.
  - Run `cargo test -p sim-harness`.
  - Ensure all tests pass; ask the user if questions arise.

- [ ] 4. Harness reusability validation
  - [ ] 4.1 Add a trivial second `StressModel` + `ExhaustiveModel` in `tools/sim-harness` tests
    - In `#[cfg(test)]`, implement a few-line throwaway model (e.g. a bounded counter with one safety invariant and one fault) against both traits and run it through `StressRunner` and `run_bounded_exhaustive` to prove the harness API carries no broker/placement-specific assumptions — the lightweight stand-in for the future admission-control (055) and connection-management (060) consumers.
    - _Requirements: 7.1, 7.2, 7.3, 7.4_

- [ ] 5. Broker model: domain types and pure state
  - [ ] 5.1 Scaffold the `tools/broker-sim` binary crate
    - Create `tools/broker-sim/Cargo.toml` (`publish = false`, `edition = "2024"`, depends on `sim-harness`; `proptest` as a dev-dependency only; no `tokeira-runtime`, no `tokio`) and `src/main.rs` plus the `model`, `events`, `invariants`, `faults`, `exhaustive`, `bug` modules.
    - _Requirements: 8.2, 34.1, 34.2, 34.3, 34.4, 35.2_
  - [ ] 5.2 Define the re-modelled domain identifiers and keys in `tools/broker-sim/src/model.rs`
  ...
  - [ ] 5.3 Define `BrokerState` and `AuthoritativePendingState` in `tools/broker-sim/src/model.rs`
  ...
  - [ ] 5.4 Define `BrokerCfg` and `WorkloadShape`
...
- [ ] 6. Broker model: events and delivery semantics
  - [ ] 6.1 Define `BrokerEvent` in `tools/broker-sim/src/events.rs`
  ...
  - [ ] 6.2 Implement publish + dedup + tier placement (`handle` for `PublishWft`/`PublishActivity`)
  ...
  - [ ] 6.3 Implement the poll tier-ladder and reservation/commit split
  ...
  - [ ] 6.4 Implement sticky promotion, grace scan, direct claim, and query path
  ...
  - [ ] 6.5 Implement WFT-vs-AT separation, control loop, and quality accounting
  ...
  - [ ] 6.6 Implement the sweeper, crash, lease expiry, and partition pressure
  ...
  - [ ] 6.7 Implement `bootstrap` (workload + fault schedule) and `StressModel` for `BrokerModel`
...
- [ ] 7. Broker model: invariants (S1–S7, L1–L4)
  - [ ] 7.1 Implement the safety invariants in `tools/broker-sim/src/invariants.rs`
  ...
  - [ ] 7.2 Implement the liveness invariants in `tools/broker-sim/src/invariants.rs`
  ...
  - [ ] 7.3 Property test: tier-selection determinism and dedup (model-level, proptest)
    - In `tools/broker-sim` `#[cfg(test)]` with workspace `proptest` (≥100 iterations): the pure tier-selection function is deterministic for a fixed state+poll, and republishing an enqueued id is suppressed. Tag `// Feature: delivery-broker-simulator, Property 2` and `... Property 6`.
    - _Requirements: 24, 28_
  - [ ] 7.4 Property test: sticky promotion and reservation/commit coupling (model-level, proptest)
    - In `tools/broker-sim` `#[cfg(test)]` with `proptest` (≥100 iterations): an expired sticky TTL always yields a general-deliverable task; a non-committing `StartTxnCommit` always returns the reserved poller and never delivers a token. Tag `// Feature: delivery-broker-simulator, Property 7` and `... Property 3`.
    - _Requirements: 25, 29_

- [ ] 8. Verification modes wiring
  - [ ] 8.1 Implement the bounded-exhaustive `BrokerActionModel` in `tools/broker-sim/src/exhaustive.rs`
...
- [ ] 9. Faults and injectable bug
  ...
  - [ ] 9.4 Test: injected bug is caught; clean config passes
...
- [ ] 10. Broker-sim mode and determinism tests
  - [ ] 10.1 Healthy-run and per-fault tests
  ...
  - [ ] 10.2 Determinism test
    - Stress mode run twice with identical flags (same seeds/ops/time/faults) produces byte-identical aggregate reports; the harness determinism property (same seed+model+fault-config → identical event sequence) holds for `BrokerModel`.
    - _Requirements: 30.4, 32.1, 32.2_

- [ ] 11. Checkpoint — full simulator
  - Run `cargo +nightly fmt --all --check`.
  - Run `cargo lint`.
  - Run `cargo test -p sim-harness` and `cargo test -p broker-sim`.
...
- Deliverable A (harness) is built and validated (tasks 1–4) before Deliverable B (broker model)
  ...
  (060) consumers.
- The harness core uses no `proptest`; the broker-sim crate uses workspace `proptest` only as a
...
- `tools/placement-sim` is left untouched; rebasing it onto `sim-harness` is out of scope here.
