...
- [ ] 1. Add metric constants and recording helpers to `tokeira-storage::metrics`
  - [ ] 1.1 Add Phase 1 metric constants (DSQL_OPERATION_DURATION_SECONDS, DSQL_OCC_CONFLICT_TOTAL, DSQL_RETRY_TOTAL, DSQL_OPERATION_TOTAL) and register in METRIC_NAMES manifest
  ...
  - [ ] 1.2 Add Phase 1 recording helpers (record_dsql_operation_duration, record_dsql_occ_conflict, record_dsql_retry, record_dsql_operation_total)
  ...
  - [ ] 1.3 Add Phase 2 metric constant (DSQL_RESERVOIR_IN_FLIGHT) and register in METRIC_NAMES
  ...
  - [ ] 1.4 Add Phase 2 recording helper (set_dsql_reservoir_in_flight)
  ...
  - [ ] 1.5 Add Phase 3 metric constants (DSQL_PROJECTION_READ_DURATION_SECONDS, DSQL_PROJECTION_BATCH_SIZE) and register in METRIC_NAMES
    - _Requirements: 10.1, 11.1, 17.1_

  - [ ] 1.6 Add Phase 3 recording helpers (record_dsql_projection_read_duration, record_dsql_projection_batch_size)
    - Implement with `partition_id` label (u32 → String)
    - Add doc comments
    - _Requirements: 10.1, 11.1_

  - [ ] 1.7 Add Phase 5 metric constants (DSQL_RESERVOIR_CONNECTION_CREATE_DURATION_SECONDS, DSQL_RESERVOIR_CONNECTION_VALIDATE_DURATION_SECONDS, DSQL_RESERVOIR_CONNECTION_AGE_SECONDS) and register in METRIC_NAMES
    - Reuse existing `DSQL_POOL_CHECKOUT_DURATION_SECONDS` for checkout wait time
    - _Requirements: 18.1, 19.1, 20.1, 21.1, 17.1_

  - [ ] 1.8 Add Phase 5 recording helpers (record_dsql_reservoir_connection_create_duration, record_dsql_reservoir_connection_validate_duration, record_dsql_reservoir_connection_age)
  ...
  - [ ] 1.9 Add Phase 6 metric constants (DSQL_RATE_LIMITER_TOKENS_REMAINING, DSQL_RATE_LIMITER_THROTTLED_TOTAL, DSQL_RATE_LIMITER_THROTTLE_DURATION_SECONDS) and register in METRIC_NAMES
    - _Requirements: 22.1, 23.1, 24.1, 17.1_

  - [ ] 1.10 Add Phase 6 recording helpers (set_dsql_rate_limiter_tokens_remaining, record_dsql_rate_limiter_throttled, record_dsql_rate_limiter_throttle_duration)
    - Add doc comments
    - _Requirements: 22.1, 23.1, 24.1_

  - [ ] 1.11 Add Phase 7 metric constants (DSQL_QUERY_DURATION_SECONDS, DSQL_ROWS_READ, DSQL_ROWS_WRITTEN, DSQL_COMMIT_RETRIES) and register in METRIC_NAMES
  ...
  - [ ] 1.12 Add Phase 7 recording helpers (record_dsql_query_duration, record_dsql_rows_read, record_dsql_rows_written, record_dsql_commit_retries)
  ...
  - [ ] 1.13 Add Phase 8 metric constant (DSQL_RESERVOIR_UTILIZATION_RATIO) and register in METRIC_NAMES
    - _Requirements: 29.1, 17.1_

  - [ ] 1.14 Add Phase 8 recording helper (set_dsql_reservoir_utilization_ratio)
    - Implement utilization ratio with the (0,0) → 0.0 edge case
    - Add doc comments explaining predictive signal purpose
    - _Requirements: 29.1, 29.2, 29.3_

  - [ ] 1.15 Add Phase 10 metric constants (DSQL_SHARD_OPERATION_TOTAL, DSQL_SHARD_CONFLICT_TOTAL, DSQL_SHARD_DURATION_SECONDS) and register in METRIC_NAMES
    - _Requirements: 33.1, 34.1, 35.1, 17.1_

  - [ ] 1.16 Add Phase 10 recording helpers (record_dsql_shard_operation, record_dsql_shard_conflict, record_dsql_shard_duration)
  ...
  - [ ] 1.17 Add Phase 11 metric constants (DSQL_CONNECTION_ERROR_TOTAL, DSQL_ERROR_CODE_TOTAL) and register in METRIC_NAMES
    - _Requirements: 36.1, 37.1, 17.1_

  - [ ] 1.18 Add Phase 11 recording helpers (record_dsql_connection_error, record_dsql_error_code)
  ...
  - [ ] 1.19 Add classify_outcome helper function and classify_connection_error / extract_sqlstate utilities
...
- [ ] 2. Checkpoint — Verify metric constants and helpers compile
  - Ensure all tests pass (`cargo test -p tokeira-storage`), ask the user if questions arise.

- [ ] 3. Add metric constants and recording helpers to `tokeira-projection::metrics`
  - [ ] 3.1 Add Phase 9 metric constants (VISIBILITY_QUERY_DURATION_SECONDS, SA_INDEX_SCAN_DURATION_SECONDS, CHECKPOINT_WRITE_DURATION_SECONDS) and register in METRIC_NAMES
  ...
  - [ ] 3.2 Add Phase 9 recording helpers (record_visibility_query_duration, record_sa_index_scan_duration, record_checkpoint_write_duration)
...
- [ ] 4. Wire recording calls into DsqlRunRepository
  - [ ] 4.1 Add operation duration and throughput recording to all DsqlRunRepository methods
  ...
  - [ ] 4.2 Add OCC conflict and retry recording to DsqlRunRepository retry paths
    - On serialization failure: record_dsql_occ_conflict
    - On retry outcome: record_dsql_retry with success/exhausted
    - _Requirements: 2.1, 2.2, 3.1, 3.2_

  - [ ] 4.3 Add query-level decomposition recording (query duration, rows read, rows written, commit retries)
  ...
  - [ ] 4.4 Add per-shard distribution recording to DsqlRunRepository
  ...
  - [ ] 4.5 Add error classification recording to DsqlRunRepository error paths
...
- [ ] 5. Checkpoint — Verify DsqlRunRepository instrumentation compiles
  - Ensure all tests pass (`cargo test -p tokeira-storage`), ask the user if questions arise.

- [ ] 6. Wire recording calls into Reservoir and DsqlConnectionDirector
  - [ ] 6.1 Add reservoir ready connections gauge updates to Reservoir checkout/return/create/retire paths
  ...
  - [ ] 6.2 Add in-flight gauge and checkout duration recording to DsqlConnectionDirector.acquire/release
...
- [ ] 7. Checkpoint — Verify Reservoir and Director instrumentation compiles
  - Ensure all tests pass (`cargo test -p tokeira-storage`), ask the user if questions arise.

- [ ] 8. Wire recording calls into DsqlProjectionLog
  - [ ] 8.1 Add projection read duration and batch size recording to DsqlProjectionLog::read_from
...
- [ ] 9. Wire recording calls into TokenBucketRateLimiter
  - [ ] 9.1 Add rate limiter metrics to TokenBucketRateLimiter acquire path
...
- [ ] 10. Implement periodic reporter task
  - [ ] 10.1 Implement spawn_periodic_reporter in DsqlConnectionDirector
  ...
  - [ ] 10.2 Wire spawn_periodic_reporter into DsqlConnectionDirector::start
    - Start the reporter task during director initialization
    - Store the JoinHandle for graceful shutdown
    - _Requirements: 9.1_

- [ ] 11. Checkpoint — Verify all recording call wiring compiles
  - Ensure all tests pass (`cargo test -p tokeira-storage`), ask the user if questions arise.

- [ ] 12. Wire recording calls into DSQL visibility and projection checkpoint paths
  - [ ] 12.1 Add visibility query and search-attribute index metrics to `DsqlVisibilityStore`
  ...
  - [ ] 12.2 Add checkpoint write latency recording to projection checkpoint writes
    - On checkpoint writes: record_checkpoint_write_duration with partition_id
    - _Requirements: 32.1, 32.2_

- [ ] 13. Checkpoint — Verify projection instrumentation compiles
  - Ensure all tests pass (`cargo test -p tokeira-projection`), ask the user if questions arise.

- [ ] 14. Update dashboard JSON
  - [ ] 14.1 Add DSQL Health stat row at the top of the dashboard
  ...
  - [ ] 14.2 Add DSQL Operations row with per-operation latency, rate by outcome, and conflict rate panels
  ...
  - [ ] 14.3 Update existing DSQL Pool row with reservoir health panels
    - Add reservoir ready connections and in-flight timeseries
    - Preserve existing lifecycle and class budget panels
    - _Requirements: 15.1, 15.2, 15.3, 15.4, 15.5, 15.6_

  - [ ] 14.4 Add Connection Lifecycle row (checkout wait, creation time, validation time, connection age)
    - Use summary quantile queries grouped by class/retirement_reason
    - _Requirements: 38.1, 38.2, 38.3, 38.4, 38.5_

  - [ ] 14.5 Add Rate Limiter row (token fill level, throttled rate, throttle duration)
    - _Requirements: 39.1, 39.2, 39.3, 39.4, 39.5_

  - [ ] 14.6 Add Query Decomposition row (SQL execution time, rows read, rows written, commit retries)
    - _Requirements: 40.1, 40.2, 40.3, 40.4, 40.5, 40.6_

  - [ ] 14.7 Add Shard Distribution row (per-shard op rate, conflict rate, latency)
    - _Requirements: 41.1, 41.2, 41.3, 41.4, 41.5_

  - [ ] 14.8 Add Predictive Signals row (utilization ratio with 0.8 threshold, refill vs retirement)
    - _Requirements: 42.1, 42.2, 42.3, 42.4_

  - [ ] 14.9 Add Error Classification row (connection errors by kind, SQLSTATE codes)
    - Include description annotations explaining common error kinds and SQLSTATE codes
    - _Requirements: 43.1, 43.2, 43.3, 43.4, 43.5_

  - [ ] 14.10 Add Projection Deep Metrics row (record lag, visibility query, SA index attribution, checkpoint write)
    - _Requirements: 44.1, 44.2, 44.3, 44.4, 44.5, 44.6_

  - [ ] 14.11 Apply consistent style conventions across all new and existing panels
...
- [ ] 15. Checkpoint — Verify dashboard JSON is valid
  - Parse the JSON to verify structural validity, ask the user if questions arise.

- [ ] 16. Property tests and unit tests
  - [ ]* 16.1 Write property test for metric name validation (Property 1)
  ...
  - [ ]* 16.2 Write property test for counter accounting accuracy (Property 2)
  ...
  - [ ]* 16.3 Write property test for histogram observation accuracy (Property 3)
  ...
  - [ ]* 16.4 Write property test for gauge last-write-wins (Property 4)
  ...
  - [ ]* 16.5 Write property test for utilization ratio computation (Property 5)
  ...
  - [ ]* 16.6 Write property test for shard ID derivation determinism (Property 6)
  ...
  - [ ]* 16.7 Write unit tests for outcome classification and error classification helpers
  ...
  - [ ]* 16.8 Write unit tests for dashboard JSON correctness
