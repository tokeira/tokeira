...
- [ ] 1. Self-assign shards and local epoch validation (critical path)
  - [ ] 1.1 Implement self-assignment loop in `build_and_serve_with_storage`
  ...
  - [ ] 1.2 Add a runtime-owned self-assignment/epoch validation seam
  ...
  - [ ] 1.3 Replace per-transition lease row SELECT with lane-local epoch validation in no-controller mode
...
- [ ] 3. vis_rollup schema migration and partition sharding
  - [ ] 3.1 Modify existing vis_rollup schema to include `partition_id`
  ...
  - [ ] 3.2 Update `accumulate_rollup` to accept and use `partition_id`
  ...
  - [ ] 3.3 Thread configured projection partition count through DSQL projection log writes and worker startup
  ...
  - [ ] 3.4 Update visibility read path to SUM across partitions
  ...
  - [ ] 3.5 Extend `ProjectionSink::apply` trait and `DsqlVisibilityStore::apply` with `partition_id`
...
- [ ] 4. OCC retry with connection release
  - [ ] 4.1 Implement OCC retry loop in `accumulate_rollup`
...
- [ ] 5. Single-transaction projection apply
  - [ ] 5.1 Restructure `DsqlVisibilityStore::apply` for single-transaction execution+search-attrs
...
- [ ] 7. Statement-level duration metrics
  - [ ] 7.1 Add `tokeira_storage_dsql_statement_duration_seconds` histogram
  ...
  - [ ] 7.2 Instrument commit path statements
  ...
  - [ ] 7.3 Instrument projection path statements
...
- [ ] 8. Class permit wait metrics
  - [ ] 8.1 Add permit wait duration and pool waiting gauge metrics
...
- [ ] 9. Generic storage metrics from DSQL path
  - [ ] 9.1 Extend `record_dsql_commit_operation!` macro to emit generic metrics
...
- [ ] 10. Default configuration for DSQL deployments
  - [ ] 10.1 Implement `apply_storage_defaults` in `TokeiraConfig`
  ...
  - [ ]* 10.2 Write property test for legacy default promotion (Property 10)
  ...
  - [ ]* 10.3 Write property test for DSQL no single-shard (Property 11)
...
- [ ] 12. Validation benchmark documentation
  - [ ] 12.1 Document the validation benchmark command and success criteria
  ...
  - [ ] 12.2 Apply dashboard style contract to DSQL performance panels
