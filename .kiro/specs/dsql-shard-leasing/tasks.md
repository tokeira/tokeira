...
- [x] 1. Add `lease_duration` to `DsqlPoolConfig` and validate
  - [x] 1.1 Add `lease_duration` field to `DsqlPoolConfig` with 30-second default
  ...
  - [x] 1.2 Write unit tests for `lease_duration` config validation
...
- [x] 2. Add `lease_duration` to `DsqlRunRepository` and update constructors
  - [x] 2.1 Add `lease_duration` field to `DsqlRunRepository` struct and update `new` / `new_with_acquirer`
  ...
  - [x] 2.2 Update `DsqlStore::from_connector` to pass `config.lease_duration`
    - Pass `config.lease_duration` as the fourth argument to `DsqlRunRepository::new`
    - _Requirements: 9.2_

  - [x] 2.3 Update existing test helpers that call `new_with_acquirer`
...
- [x] 4. Extract pure lease decision helpers
  - [x] 4.1 Implement `epoch_to_sql` and `epoch_from_sql` checked conversion helpers
  ...
  - [x] 4.2 Implement `interpret_acquire` pure helper function
  ...
  - [x] 4.3 Implement `decide_renew` pure helper function
...
- [x] 5. Implement `LeaseRepository for DsqlRunRepository`
  - [x] 5.1 Implement `try_acquire_bundle`
    ...
    - If the INSERT affected 1 row, skip the UPDATE; SELECT epoch back and call `interpret_acquire(insert_rows_affected = 1, update_rows_affected = 0, acquired_epoch = Some(epoch), rejected_row = None)`
    - If the INSERT affected 0 rows, execute `UPDATE shard_lease SET owner = $2, epoch = CASE WHEN owner = $2 AND lease_expiry > $4 THEN epoch ELSE epoch + 1 END, lease_expiry = $3 WHERE shard_id = $1 AND (owner = $2 OR lease_expiry <= $4)` with `$4 = app_now` — no SQL `now()`
    - If the UPDATE affected 1 row, SELECT epoch back and call `interpret_acquire(insert_rows_affected = 0, update_rows_affected = 1, acquired_epoch = Some(epoch), rejected_row = None)`
    - If both INSERT and UPDATE affected 0 rows, SELECT owner+epoch and call `interpret_acquire` with rejection data
...
- [x] 6. Checkpoint — Ensure compilation and existing tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Property-based tests for lease decision logic
  - [x] 7.1 Write property test for epoch monotonicity on acquire (Property 1)
  ...
  - [x] 7.2 Write property test for active lease single-writer rejection (Property 2)
  ...
  - [x] 7.3 Write property test for renewal fidelity (Property 3)
  ...
  - [x] 7.4 Write property test for ShardEpoch round-trip (Property 4)
...
- [x] 8. Unit tests for specific behaviors
  - [x] 8.1 Write unit test for `DbClass::Control` routing
  ...
  - [x] 8.2 Write unit test for absent-lease renewal rejection
    - Verify `decide_renew(None, ...)` returns `Rejected { current_owner: "", current_epoch: ShardEpoch::ZERO }`
    - _Requirements: 7.1_

  - [x] 8.3 Write unit test for `shard_id_to_uuid` determinism in lease binding
  ...
  - [x] 8.4 Write unit test for OCC error classification
...
- [x] 9. Integration tests (gated behind `dsql-integration` feature)
  - [x] 9.1 Integration test: acquire → renew → expire → takeover cycle
  ...
  - [x] 9.2 Integration test: concurrent acquire — exactly one succeeds
  ...
  - [x] 9.3 Integration test: epoch fence integration with expired takeover
  ...
  - [x] 9.4 Integration test: INSERT ON CONFLICT race on first acquire
  ...
  - [x] 9.5 Integration test: active same-owner reacquire is idempotent
