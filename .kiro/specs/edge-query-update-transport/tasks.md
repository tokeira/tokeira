...
- [x] 11. Fix query-only WFT to include workflow history
  - [x] 11.1 Ensure query-only poll responses include the workflow's committed history
    - When constructing a synthetic query-only WFT, load the workflow's history from storage and include it in the poll response
    - Without history, the worker evaluates queries against initial state (counter=0 instead of counter=5)
...
- [x] 12. Implement `PollWorkflowExecutionUpdate` long-poll
  - [x] 12.1 Implement `poll_workflow_execution_update` edge method and gRPC handler
    - Look up the update in the `UpdateRegistry` by `(run_key, update_id)`
    - Wait for the resolution with a timeout (default 60s)
    - Return the update outcome (accepted/completed/rejected) or empty response on timeout
    - Replace the `Status::unimplemented` stub in `grpc/workflow_service.rs`
