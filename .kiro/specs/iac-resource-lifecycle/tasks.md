...
- [x] 1. Add `DestroyMode` marker and `remove_extension` to `tokeira-iac`
  - [x] 1.1 Define `DestroyMode` and extend `ProvisionContext` API
  ...
  - [x]* 1.2 Write property test for `DestroyMode` visibility through `ModuleContext` (Property 1)
  ...
  - [x]* 1.3 Write unit test for `ProvisionContext::remove_extension`
...
- [x] 2. Add `Engine::plan_destroy` and wire orchestrator destroy paths
  - [x] 2.1 Implement `Engine::plan_destroy` and `Engine::plan_destroy_for_modules` in `tokeira-iac`
  ...
  - [x] 2.2 Register `DestroyMode` in the orchestrator and scope it to the operation
  ...
  - [x]* 2.3 Write unit test for `DestroyMode` registration and scoping
...
- [x] 3. Checkpoint — Ensure workspace compiles
...
- [x] 4. Implement CAS-aware incremental state saving
  - [x] 4.1 Update `StateStore::save` to return the new version
  ...
  - [x] 4.2 Rewrite `make_saver` in `tokeira-orchestrator` for CAS tracking
  ...
  - [x] 4.3 Remove the final `state_store.save` call from `apply` and `destroy`
  ...
  - [x] 4.4 Save per pruned resource during `refresh_state` (when a saver is available)
  ...
  - [x] 4.5 Audit `apply_changes` and `destroy_changes` saver invocations
  ...
  - [x]* 4.6 Write property test for `StateSaver` invocation count (Property 2)
    - **Property 2: StateSaver Invocation Count**
    - **Validates: Requirements 2.1, 2.2, 2.3, 2.5, 2.6**
    - Use `proptest` to generate random sequences of N resources with a mix of create/update/delete changes and M stale managed resources whose `describe()` returns `None` during refresh
    - Use an `Arc<AtomicUsize>` counter in the `StateSaver` callback
    - After apply or destroy completes, assert `counter == N + M`, where N is the number of successful non-NoChange lifecycle operations and M is the number of per-resource refresh prunes
    - Include a focused regression with at least two stale managed resources in one refresh cycle to prove the saver is invoked once per pruned resource rather than once per refresh
    - Minimum 100 iterations
    - Test location: `crates/tokeira-iac/src/engine.rs` `#[cfg(test)]` module

  - [x]* 4.7 Write property test for `StateSaver` error aborts engine (Property 3)
  ...
  - [x]* 4.8 Write unit test for CAS version tracking across saves
...
- [x] 5. Formalise module-scoped delete filtering
  - [x] 5.1 Audit `filter_changes_by_modules` behavior
  ...
  - [x]* 5.2 Write property test for module-scoped delete filtering (Property 4)
...
- [x] 6. Formalise describe-before-delete safety
  - [x] 6.1 Audit `destroy_changes` for describe-before-delete
  ...
  - [x]* 6.2 Write property test for describe-before-delete count (Property 7)
  ...
  - [x]* 6.3 Write property test for describe-before-delete uses live state (Property 8)
...
- [x] 7. Checkpoint — Ensure engine tests pass
  - Run `cargo test -p tokeira-iac -p tokeira-orchestrator` and `cargo lint` and verify all tests pass

- [x] 8. Extend `ProvisionContext` with completion/failure progress callbacks
  - [x] 8.1 Add new progress callback fields and setters to `ProvisionContext`
  ...
  - [x] 8.2 Call completion/failure callbacks from `apply_changes` and `destroy_changes`
  ...
  - [x]* 8.3 Write unit tests for new callbacks
...
- [x] 9. Implement CLI progress reporting with `indicatif` and `console`
  - [x] 9.1 Add `console` and `indicatif` dependencies to `apps/tkr`
    - Add `console = "0.15"`, `indicatif = "0.17"`, and `tracing.workspace = true` to `apps/tkr/Cargo.toml`
    - `tracing` is required because `emit_json_line` logs serialization failures with `tracing::warn!`; do not rely on transitive dependencies
  ...
  - [x] 9.2 Create the `tui` module in `apps/tkr`
  ...
  - [x] 9.3 Implement `ActionTuiHandle::install` wiring all five reporters
  ...
  - [x] 9.4 Implement `ActionTuiHandle::print_summary`
  ...
  - [x] 9.5 Thread `cli.json` into `commands::infra::run` and wire the TUI
  ...
  - [x]* 9.6 Write property test for JSON event well-formedness (Property 9)
  ...
  - [x]* 9.7 Write property test for progress counter accuracy (Property 10)
  ...
  - [x]* 9.8 Write unit tests for terminal vs non-terminal fallback
...
- [x] 10. Checkpoint — Ensure CLI compiles and tests pass
...
- [x] 11. Formalise config writeback behavior
  - [x] 11.1 Audit `write_tokeirad_writeback`
  ...
  - [x]* 11.2 Write property test for TOML writeback round-trip (Property 5)
  ...
  - [x]* 11.3 Write property test for comment preservation (Property 6)
...
- [x] 12. Document and verify the `effective_managed` convention
  - [x] 12.1 Add a doc-only module in `tokeira-iac` describing the convention
  ...
  - [x] 12.2 Extract `effective_managed` as a pure helper in the DSQL cluster resource
  ...
  - [x]* 12.3 Write unit tests for `effective_managed` covering all four combinations
