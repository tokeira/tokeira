...
4. THE Dagger pipeline SHALL build the `tokeirad` binary with `cargo build --release --bin tokeirad --target <target-triple>` using the Target_Architecture supplied in the build request. Target triples are glibc gnu variants, not musl — see Req 3.3 acceptance criterion 3 for the target-triple specification.
