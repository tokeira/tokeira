...
PRE-STEP (decided, do it first): Change the config types in platforms/compose-syn/src/definition.rs to the flat 003 §6 shape, because fidelity.rs:17,93 import `definition::DsqlMode` and `definition::Storage` directly and the flatten must leave the .tkd surface:
  enum DsqlMode { Managed, Preexisting }
