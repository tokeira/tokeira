...
- A new `remote-workstation` IaC module in `crates/tokeira-aws/src/resources/` following the `tokeira-iac` `Module`/`Resource` pattern established by `temporal-dsql-deploy-eks`. The module declares one EC2 instance, one security group with no inbound rules, persistent cache and repo EBS volumes, an IAM role (+ instance profile) granting SSM agent permissions, and a tag vocabulary that marks instances as Tokeira workstations.
...
- [`temporal-api-v1.62-sync`](../temporal-api-v1.62-sync/requirements.md): the spec whose cold-rebuild tax motivated this one. Its deferred tasks (facade + v0.4 SDK integration test) are intended to complete on the remote workstation this spec ships.
...
- `tokeira-iac`: this spec's IaC module is a consumer of the `Module` / `Resource` / `diff` / `apply` engine already established by `temporal-dsql-deploy-eks`. No changes to `tokeira-iac` itself are required.
- `tokeira-aws`: gains one new resource file (`remote_workstation.rs`) and one new AWS SDK client dependency (`aws-sdk-ssm`). Existing resource implementations are unchanged.
