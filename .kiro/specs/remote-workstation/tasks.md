...
3. SSM surface — `ssh` (aws CLI subprocess) and `remote_exec` (`SendCommand` polling with bounded near-real-time output).
4. CLI handlers in `apps/tkr` and clap surface extension in `cli.rs`. Thin wrappers over the engine.
5. Correctness-property tests and documentation.

Task groups 1–4 plus the cloud-init bootstrap/watchdog tasks are the v1 "you can build Tokeira on this" deliverable. Property tests and documentation close the spec; if an MVP cut is needed, only optional property/documentation work may defer.
...
- [x] 1. Engine-layer skeleton in `tokeira-aws`
  - [x] 1.1 Add `aws-sdk-ssm = "1"` to `crates/tokeira-aws/Cargo.toml` dependencies
    ...
    - (Req 6.2.3)
  - [x] 1.2 Declare `pub mod remote_workstation` in `crates/tokeira-aws/src/lib.rs`
    ...
    - (Req 6.3.1)
  - [x] 1.3 Implement the public type surface in `remote_workstation.rs`
  ...
  - [x] 1.4 Implement `Workstation::new(region) -> Result<Self, WorkstationError>`
    ...
    - (Req 6.2.1)
  - [x] 1.5 Stub all lifecycle methods as `unimplemented!()`
...
- [x] 3. Lifecycle methods implemented
  - [x] 3.1 Implement `Workstation::list` — baseline discovery
    ...
    - (Req 1.5.2)
  - [x] 3.2 Implement `Workstation::status` for an existing workstation
  ...
  - [x] 3.3 Implement `Workstation::up` — the core engine method
    - Step 1: Discover — call `ec2.describe_instances` with the discovery filter from Req 1.1.1. If `--workstation` is supplied, include the `tag:workstation-id=<id>` filter. Enumerate matches.
    - Step 2a: Zero matches → fresh create path per design.md §3.1.
      - Use the caller-supplied workstation ID when present; otherwise generate `workstation_id` via `ulid::Ulid::new()`.
      - Verify no tagged workstation resource already uses the chosen ID before creating anything.
      ...
      - Render bootstrap script via `remote_workstation_bootstrap::render(BootstrapContext)` with the Cache_Volume and Repo_Volume IDs in the context.
      - `ec2.run_instances` with AMI, subnet, security group, instance profile, root-volume block-device mapping, user-data, and explicit primary-network-interface public IPv4 association in default public-subnet mode.
      - Wait for instance state `Running` (use `ec2.describe_instances` polling up to 5-minute timeout).
      - Attach Cache_Volume to `/dev/sdf`, Repo_Volume to `/dev/sdg` via `ec2.attach_volume`; the bootstrap waits for those volume IDs to appear under `/dev/disk/by-id`.
    ...
    - (Req 1.1.1 through 1.1.8, 4.1.3, 7.1)
  - [x] 3.4 Implement `Workstation::stop`
    ...
    - (Req 1.2, 4.1.3)
  - [x] 3.5 Implement `Workstation::destroy`
    ...
    - (Req 1.3, 10.2.8)
  - [x] 3.6 Implement `Workstation::bootstrap` (drift-refresh)
    ...
    - (Req 1.4, 7.2)
  - [x] 3.7 Implement `Workstation::idle_defer`
...
- [ ] 4. Checkpoint — lifecycle functional end-to-end against a mocked AWS
  - All unit tests (mocked SDK clients) pass. A live-AWS smoke test is deferred to task 9 but `cargo check && cargo test` is green for the mocked path.

- [x] 5. SSM surface — `ssh` and `remote_exec`
  - [x] 5.1 Implement `Workstation::remote_exec`
    ...
    - Poll `ssm.get_command_invocation` at 500 ms intervals. On each poll, diff `StandardOutputContent` and `StandardErrorContent` against the last-seen values and write newly observed deltas to the caller's writers. This is the bounded near-real-time v1 contract, not true byte-stream terminal output.
  ...
  - [x] 5.2 Verify `session-manager-plugin` presence
    ...
    - (Req 4.3.2)
  - [x] 5.3 Implement `Workstation::start_interactive_session` helper for `ssh`
    - Resolve instance ID.
...
- [x] 6. CLI layer — `tkr workstation` subcommand group
  - [x] 6.1 Extend `apps/tkr/src/cli.rs`
  ...
  - [x] 6.2 Create `apps/tkr/src/commands/workstation/mod.rs`
    ...
    - (Req 6.1.3, 7.1.3)
  - [x] 6.3 Implement handler `apps/tkr/src/commands/workstation/up.rs`
    ...
    - (Req 1.1, 1.7)
  - [x] 6.4 Implement `stop.rs`
    - `resolve_workstation_id` → `workstation.stop(id)`.
    - (Req 1.2)
  - [x] 6.5 Implement `destroy.rs`
  ...
  - [x] 6.7 Implement `remote_exec.rs`
    ...
    - (Req 4.4, 10.3)
  - [x] 6.8 Implement `status.rs`, `list.rs`, `bootstrap.rs`, `idle.rs`
    ...
    - (Req 5.2.1, 5.2.3)
  - [x] 6.9 Dispatch in `apps/tkr/src/main.rs`
    ...
    - (Req 6.1.4)
  - [x] 6.10 Create `apps/tkr/src/commands/workstation/secret_scan.rs`
    ...
    - (Req 10.3.1)
  - [x] 6.11 Implement `apps/tkr/src/commands/workstation/github_key.rs`
    - `Add` handler:
...
- [ ] 7. Checkpoint — CLI end-to-end smoke test
...
- [x] 8. Cloud-init bootstrap + idle-shutdown watchdog
  - [x] 8.1 Create `crates/tokeira-aws/src/remote_workstation_bootstrap.rs`
    - `BootstrapContext` struct per design.md §5, including explicit Cache_Volume and Repo_Volume IDs so bootstrap does not need EC2 `DescribeVolumes`.
    ...
    - Phase 1 (filesystem): NVMe instance-store detect via `/dev/disk/by-id/nvme-Amazon_EC2_NVMe_Instance_Storage*`, format if unformatted, mount at `/work`, wait for Cache_Volume and Repo_Volume by `/dev/disk/by-id/nvme-Amazon_Elastic_Block_Store_<volume-id>`, then write `fstab` entries for EBS volumes.
    ...
    - Phase 4 (repo clone): if `/work/repo/tokeira/.git` missing, clone; if clone fails, write `/etc/tokeira/repo-clone-status` and continue so private repos can be repaired with `tkr workstation github-key add`; symlink `/work/tokeira` when the checkout exists.
  ...
  - [x] 8.2 Write the idle-shutdown watchdog script
    ...
    - (Req 5.1)
  - [x] 8.3 Idempotency verification
...
- [ ] 9. Checkpoint — live-AWS smoke test
...
- [ ] 10. Correctness-property tests
  ...
  - [ ]* 10.2 Property 2 — destroy is total (Req 9.2)
    ...
    - Min 32 iterations.
  - [x]* 10.3 Property 3 — fingerprint determinism (Req 9.3)
...
- [x] 11. Documentation updates
  ...
  - [x] 11.2 Add `docs/remote-workstation.md`
  ...
  - [x] 11.3 Update `CONTRIBUTING.md` if relevant
...
- [ ] 13. Feature 10 — GitHub credential policy tests
  - [x]* 13.1 Secret scanner coverage
    ...
    - (Req 10.3.1, 10.3.2)
  - [x]* 13.2 Deploy-key registry round-trip
    ...
    - (Req 10.2.8)
  - [x]* 13.3 Host-key structural validity
