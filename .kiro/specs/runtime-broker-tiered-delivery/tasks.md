...
ground-truthed to v1.31.0 (cite path + tag inline). Boxes are unchecked for the implementer (Codex).

## Tasks

- [ ] 1. Reproduce the gap (write failing tests first)
  - [ ] 1.1 Runtime test: a query dispatched to a **quiescent** run with a polling worker is delivered
  ...
  - [ ] 1.2 Confirm the consistent-query (buffered) path passes today (guard against regression).
    - _Requirements: 4.3, 7.3_

- [ ] 2. Query-task representation on the poll response
  - [ ] 2.1 Add a query-task variant to `PollWorkflowTaskQueueResponse` in
  ...
  - [ ] 2.2 Map it in `crates/tokeira-edge/src/grpc/translate.rs` to the proto
...
- [ ] 3. Deliver queries on the workflow-task poll
  - [ ] 3.1 Choose delivery shape per design §1: (A) unified broker poll result (preferred) or
    (B) edge-level `select!` over `poll_workflow_task` + a new `poll_query`. Document the choice.
    - _Requirements: 1.1, 1.2, 1.5_
  - [ ] 3.2 Implement query-task poll assembly: build the response from run state — query task token,
  ...
  - [ ] 3.3 Register the caller's `response_tx` in `PendingQueryStore` keyed by
...
- [ ] 4. Sticky-first routing with normal-queue fallback
  - [ ] 4.1 Route a query to the sticky tier first when the run has live sticky affinity; add a
    per-query sticky schedule-to-start deadline.
    - _Requirements: 2.1_
  - [ ] 4.2 On sticky-deadline expiry, clear `sticky_preferred` so any matching worker can take the
  ...
  - [ ] 4.3 No sticky affinity → deliver on the live tier directly.
    - _Requirements: 2.3_

- [ ] 5. Checkpoint — query delivery green
  - Run `cargo test -p tokeira-runtime -p tokeira-edge`; the task 1.1 repro now passes; 1.2 still passes.

- [ ] 6. Make broker tiers explicit (refactor)
  - [ ] 6.1 Name the sticky / live / backlog tiers in `broker.rs`, folding query readiness into the
  ...
  - [ ] 6.2 Preserve workflow-task delivery semantics: identical sticky-vs-live take rules, no
    double-delivery.
    - _Requirements: 5.3, 6.4_

- [ ] 7. Property + regression tests
  - [ ] 7.1 Property: at-most-once query answer (sticky→live fallback never double-delivers).
    - _Requirements: 6.2, 7.4_
  - [ ] 7.2 Property: tier promotion (sticky-expiry → live) for both workflow tasks and queries.
    - _Requirements: 5.2, 7.4_
  - [ ] 7.3 Regression: `accepted_update_is_not_redelivered_as_pending_transport` still passes; the
    consistent-query buffered path still passes.
    - _Requirements: 6.3, 4.3, 7.3_

- [ ] 8. Checkpoint — full suite, lint, fmt
  - `cargo +nightly fmt --all --check`, `cargo lint`, `cargo test-lint`,
    `cargo test -p tokeira-runtime -p tokeira-edge`.

- [ ] 9. Acceptance (operator-run, not a unit test)
