...
- [ ] 1. Update core routing functions in `scanner.rs`
  - [ ] 1.1 Change `lane_index_for` to accept `ShardId` instead of `RunKey`
  ...
  - [ ] 1.2 Change `pick_lane` to accept `ShardId` instead of `RunKey`
    - Add `debug_assert!(!lanes.is_empty())` and `debug_assert_eq!(lanes.len(), lane_count.max(1))`
    - _Requirements: 1.3_
  - [ ]* 1.3 Write property test: shard-to-lane computation correctness
  ...
  - [ ]* 1.4 Write property test: lane index bounds
  ...
  - [ ]* 1.5 Write unit tests for core routing functions
...
- [ ] 2. Migrate scanner callers to use `ShardId`
  - [ ] 2.1 Update `run_timer_scanner` closure in `scanner.rs`
    - Replace `due.run_key` with captured `shard_id` from `for shard_id in active_shards` loop
    - _Requirements: 3.1, 3.2_
  - [ ] 2.2 Update `run_wft_timeout_scanner` closure in `wft_timeout.rs`
    - Replace `entry.run_key` with `entry.shard_id` from `WftTimeoutEntry`
    - _Requirements: 4.1_
  - [ ] 2.3 Update `scan_activity_timeouts_once` in `activity_timeout.rs`
    - Replace `entry.run_key` with `entry.shard_id` from `ActivityTrackingEntry` (works for both shard-filtered and unfiltered scans)
    - _Requirements: 5.1_
  - [ ] 2.4 Update nexus timeout scanner closure in `nexus.rs`
    - Replace `entry.run_key` with `entry.shard_id` from `NexusTimeoutEntry`
    - _Requirements: 6.1_
  - [ ] 2.5 Update `run_workflow_timeout_scanner` closure in `timeout.rs`
    - Replace `entry.run_key` with `entry.shard_id` from tracking entry
    - _Requirements: 6a.1_
  - [ ] 2.6 Update `sweep_shard` due-timer loop in `recovery.rs`
    - Replace `due.run_key` with `shard_id` parameter (already the first arg of `sweep_shard`)
    - _Requirements: 7.1, 7.2_

- [ ] 3. Migrate publisher and runtime callers to derive `ShardId`
  - [ ] 3.1 Update `RuntimeDispatchPublisher::pick_lane` in `publisher.rs`
  ...
  - [ ] 3.2 Update `TokeiraRuntime::pick_lane` in `runtime.rs`
  ...
  - [ ]* 3.5 Write property test: end-to-end routing determinism
  ...
  - [ ]* 3.6 Write property test: RunKey-to-lane affinity through shard derivation
  ...
  - [ ]* 3.7 Write unit tests for publisher and runtime routing
...
- [ ] 5. Update architecture documentation
  - [ ] 5.1 Update `docs/architecture/005-decisions-and-boundaries.md`
