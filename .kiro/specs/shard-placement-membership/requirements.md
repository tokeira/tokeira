...
4. **Routing granularity is finer than lease granularity.** Queue partitions (large space) provide even spread and cheap rebalance. Lease bundles (smaller space) keep authoritative lease renewal cheap.
...
- Dynamic placement policy (when to move queue partitions) — separate spec per 037-dynamic-placement
...
- **Queue_Partition**: A placement unit derived from `hash(placement_key) mod N`. Queue partitions are the main unit of dynamic placement — finer-grained than bundles.
- **Queue_Family**: Identified by `(namespace_id, task_queue, task_kind)`. A queue family contains multiple queue partitions.
- **Queue_Home**: The placement preference for a queue family partition — determines where workers should poll and where workflow/activity tasks are dispatched. Queue-home is an optimisation for poll/task locality, not an execution correctness boundary.
...
- **Routing_Snapshot**: A compact data structure published by the controller containing `execution_bundle_owners`, `queue_partition_homes`, `node_endpoints`, `placement_config`, and a generation counter. Advertises only DSQL-confirmed actual ownership.
- **Incarnation_Id**: A UUID identifying a runtime node incarnation. Assigned at startup and stable only for the lifetime of the process. Previously called `NodeId` — the name clarifies it is not a durable node identity.
...
4. WHEN mutating the generation counter, THE Placement_Controller SHALL use a DSQL CAS operation (`UPDATE ... WHERE generation = $expected RETURNING generation`) so that only one controller succeeds per increment.
...
3. THE heartbeat message SHALL include lane pressure metrics: active actor count, lane utilization.
...
2. THE `list_bundle_leases` method SHALL return each lease's `bundle_id`, `owner_node_id`, `epoch`, and `lease_until` timestamp.
3. THE DsqlRunRepository SHALL implement `list_bundle_leases` by querying `control.bundle_lease`.
4. THE InMemoryStore SHALL implement `list_bundle_leases` by iterating its `bundle_leases` HashMap.
...
2. WHEN `relinquish_bundle` is called, THE LeaseRepository SHALL clear the `owner_node_id` and advance the epoch, making the bundle available for acquisition by another node.
...
2. THE Routing_Snapshot SHALL contain `queue_partition_homes: HashMap<QueuePartition, QueuePartitionHome>` for all queue partitions, where `QueuePartitionHome` includes `node_id` and `bundle_id`.
...
4. THE Routing_Snapshot SHALL contain a `node_endpoints: HashMap<Incarnation_Id, NodeEndpoint>` map so that edges can resolve node IDs to concrete network addresses.
...
1. THE generation counter SHALL be persisted in a dedicated DSQL row with CAS protection: `UPDATE routing_generation SET generation = generation + 1 WHERE generation = $expected_generation RETURNING generation`.
...
2. WHEN the Edge_Node receives a `NotShardOwner` error with a `current_owner_node_id` hint, THE Edge_Node SHALL first attempt to route directly to the suggested owner.
...
1. THE Edge_Node SHALL resolve queue-home for a poll request by looking up `(namespace_id, task_queue, task_kind, queue_partition)` in the Routing_Snapshot's `queue_partition_homes` map.
2. THE Edge_Node SHALL resolve queue-home for task dispatch by deriving the queue partition from the Placement_Key and looking up the preferred home.
3. WHEN no queue-home preference exists for a partition, THE Edge_Node SHALL fall back to a default routing strategy (round-robin across available runtime nodes).
...
2. THE `RoutingSnapshot` type SHALL contain `queue_partition_homes: HashMap<QueuePartition, QueuePartitionHome>` where `QueuePartitionHome` includes `node_id: IncarnationId` and `bundle_id: BundleId`.
...
3. THE Placement_Controller SHALL stop assigning new queue-home preferences to a node in `DRAINING` state.
...
2. WHEN a Runtime_Node receives a `ConnectionBudgetDirective`, THE Runtime_Node SHALL cap its reservoir `target_ready` to `min(configured_target_ready, max_reservoir_size)`.
3. IF the current reservoir size exceeds the new cap, THE Runtime_Node SHALL proactively retire excess connections.
