...
1. THE design document for this spec SHALL contain an "Implementation & Escalation Matrix" table with columns: `Field Qualified Name`, `Edge DTO Change`, `Kernel Impact` (one of `none`, `new transition variant`, `existing transition field`), `Runtime Impact` (one of `none`, `new broker state`, `existing broker state`, `new timer behaviour`), `Projection Impact` (one of `none`, `new visibility column`, `existing visibility column`, `new search attribute`), and `Implementation Notes`. The table SHALL carry three row kinds:
   - **In scope (wire-through)**: one row for every `Classification_WireThrough` field in the Surface_Audit. These are the rows that ship code in this spec. `Implementation Notes` starts with `In scope`.
   ...
   - **Classified Deferred**: one row for every `Classification_WireThrough` field that escalated to `Classification_Deferred` because its Kernel, Runtime, or Projection impact exceeded the single-file / no-kernel-change budget. Their Edge DTO, Kernel, Runtime, and Projection columns all read `none` because this spec explicitly drops the field (Req 2.2.6) and emits the protobuf default on the response path; `Implementation Notes` starts with `**Classified Deferred**` and names the follow-up spec that will reintroduce the field with typed DTO and runtime semantics.
2. WHERE an `In scope (wire-through)` row is recorded with `none` in all of Kernel Impact, Runtime Impact, and Projection Impact columns, THE implementation SHALL treat that field as edge-only propagation: the Edge_DTO gains the field, the translator decodes it, Workflow_Service_Impl consumes it at the edge boundary, and no further plumbing is required.
3. WHERE an `In scope (wire-through)` row is recorded with a non-`none` Kernel Impact, THE implementation SHALL NOT land the field in this spec — instead, the field's row SHALL be escalated to `Classified Deferred` with a pointer to the kernel-facing spec that owns its semantics. This spec SHALL NOT introduce new kernel transition variants.
4. WHERE an `In scope (wire-through)` row is recorded with a non-`none` Runtime Impact, THE implementation SHALL land the field in this spec only if the runtime change fits within a single-file modification in `crates/tokeira-runtime/`. IF the runtime change spans more than one file or introduces new runtime state types, THEN the field's row SHALL be escalated to `Classified Deferred` with a pointer to a runtime-facing spec.
5. WHERE an `In scope (wire-through)` row is recorded with a non-`none` Projection Impact, THE implementation SHALL land the field in this spec only if the projection change is a pure additive column or search attribute. IF the projection change requires a migration file against the visibility store, THEN the field's row SHALL be escalated to `Classified Deferred` with a pointer to a projection-facing spec.
