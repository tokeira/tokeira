...
3. WHEN a reviewer inspects the Surface_Audit, THE count of rows classified as Classification_WireThrough SHALL match the count of in-scope wire-through rows in the Implementation & Escalation Matrix — classifications and implementation work are in 1:1 correspondence. This count covers every counted surface kind (RPC, Field, Message, Enum, Package) that ships code in this spec, not only translator-only field updates.
...
3. WHERE the v1.62 `PauseActivityRequest` / `UnpauseActivityRequest` / `ResetActivityRequest` / `UpdateActivityOptionsRequest` messages gained new fields relative to their `*ById` predecessors, THE Surface_Audit SHALL list those fields and classify them individually. The rename migration SHALL proceed only after every such field is implemented according to its classification; fields classified Classification_WireThrough are part of the rename work and are wired through by Requirement 4.3 and the Implementation & Escalation Matrix.
...
2. WHERE a translator intentionally does not preserve a field byte-for-byte, THE property test SHALL compare the subset of fields the translator owns and SHALL explicitly exclude the non-preserved fields with a comment naming the Surface_Audit row that justifies the exclusion. The sole non-preserved case in this spec is `Classification_Deferred` fields, which are explicitly dropped on the request path per tightened Requirement 2.2.6 and re-emitted on the response path as the protobuf default value — they are NOT represented on any DTO, so the round-trip property cannot compare them in either direction.
...
### Requirement 5.1: Implementation & Escalation Matrix

**User Story:** As a Tokeira developer, I want a single implementation-scope view that records, per surface decision, whether the work is in-scope wire-through, in-scope no-op handler, or deferred-with-escalation-note, so that implementation scope and review expectations are clear at a glance.

#### Acceptance Criteria

1. THE design document for this spec SHALL contain an "Implementation & Escalation Matrix" table with columns: `Qualified Name`, `Edge DTO Change`, `Kernel Impact` (one of `none`, `new transition variant`, `existing transition field`), `Runtime Impact` (one of `none`, `new broker state`, `existing broker state`, `new timer behaviour`), `Projection Impact` (one of `none`, `new visibility column`, `existing visibility column`, `new search attribute`), and `Implementation Notes`. The table SHALL carry three row kinds:
   - **In scope (wire-through)**: one row for every counted `Classification_WireThrough` Surface_Audit row, across RPC, Field, Message, Enum, and Package kinds. These are the rows that ship code in this spec. `Implementation Notes` starts with `In scope`.
   - **In scope (no-op handler)**: one row for `RecordWorkerHeartbeat` capturing the handler work described in §9 (validation, logging, the Commit_214895e migration). `Implementation Notes` starts with `In scope (no-op handler)`. Other `Classification_NoOp` rows (e.g. the `temporal.api.worker.v1` sub-messages with `compile-only; no DTO/translator work` disposition) are NOT included because no implementation row corresponds to them.
   - **Classified Deferred**: one row for every Surface_Audit row that would otherwise be in-scope wire-through but escalated to `Classification_Deferred` because its Kernel, Runtime, or Projection impact exceeded the single-file / no-kernel-change budget. Their Edge DTO, Kernel, Runtime, and Projection columns all read `none` because this spec explicitly drops the field or surface (Req 2.2.6 for fields) and emits the protobuf default on response paths where applicable; `Implementation Notes` starts with `**Classified Deferred**` and names the follow-up spec that will reintroduce the field or surface with typed DTO and runtime semantics.
2. WHERE an `In scope (wire-through)` row is recorded with `none` in all of Kernel Impact, Runtime Impact, and Projection Impact columns, THE implementation SHALL treat that surface as edge-only propagation: the Edge_DTO or edge handler gains the required shape, the translator or handler decodes it where applicable, Workflow_Service_Impl consumes it at the edge boundary, and no further plumbing is required.
3. WHERE an `In scope (wire-through)` row is recorded with a non-`none` Kernel Impact, THE implementation SHALL NOT land the surface in this spec — instead, the row SHALL be escalated to `Classified Deferred` with a pointer to the kernel-facing spec that owns its semantics. This spec SHALL NOT introduce new kernel transition variants.
4. WHERE an `In scope (wire-through)` row is recorded with a non-`none` Runtime Impact, THE implementation SHALL land the surface in this spec only if the runtime change fits within a single-file modification in `crates/tokeira-runtime/`. IF the runtime change spans more than one file or introduces new runtime state types, THEN the row SHALL be escalated to `Classified Deferred` with a pointer to a runtime-facing spec.
5. WHERE an `In scope (wire-through)` row is recorded with a non-`none` Projection Impact, THE implementation SHALL land the surface in this spec only if the projection change is a pure additive column or search attribute. IF the projection change requires a migration file against the visibility store, THEN the row SHALL be escalated to `Classified Deferred` with a pointer to a projection-facing spec.
