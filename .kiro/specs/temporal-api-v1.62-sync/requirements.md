...
6. FOR every field classified as Classification_Deferred, THE translator SHALL decode the field into an Edge_DTO so that the edge layer preserves the bytes, but SHALL NOT propagate it into Runtime, Kernel, or Projection — the DTO field SHALL remain unread by downstream code, and a comment on the DTO field SHALL name the deferring spec.
...
2. THE implementation SHALL delegate to a new method on the `ScheduleStore` trait in `tokeira-runtime` (see `crates/tokeira-runtime/` for the existing trait) named `count_schedules(&self, namespace: NamespaceId, query: Option<&str>) -> anyhow::Result<u64>`. The method SHALL be implemented on the existing in-memory `ScheduleStore::default()` backing.
...
2. THE Workflow_Service_Impl handlers for `StartActivityExecution`, `DescribeActivityExecution`, `PollActivityExecution`, `ListActivityExecutions`, `CountActivityExecutions`, `RequestCancelActivityExecution`, `TerminateActivityExecution`, `DeleteActivityExecution` SHALL appear as a contiguous block bracketed by `// === Activity Executions — deferred to activity-executions spec ===` and `// === End Activity Executions block ===`.
