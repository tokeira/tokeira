...
| C4a | Nexus endpoint **admin CRUD** RPCs | ✅ | impl (17 pending measure) | **IMPLEMENTED (reconciled 2026-06-22)** — `api-conformance-nexus-admin` landed: live store-backed `NexusEndpointStore`/registry (`runtime/src/nexus.rs`), `NexusEndpointAdmin` validation/translation + 5 RPCs wired (`edge/src/nexus_endpoint.rs`, `grpc/operator_service.rs`), shared store wired in `tokeirad`. 13/13 edge unit tests pass. Odori-ready (runtime `CreateNexusEndpoint` resolves for dispatch). Pending: operator measure of `TestNexusEndpointsFunctionalSuite` (15) + `TestNexusAPIValidationTestSuite` (2); proptests P1–P4 not authored (example coverage exists) |
| C4b | Nexus **operation execution** / task transport | 🟡 | 2 suites unmeasured | Task transport + dispatch + kernel ops + HTTP client all landed (`edge-nexus-task-transport`, `runtime-nexus-dispatch`, `kernel-nexus-operations`, `runtime-nexus-http-client` — all tasks done). Sync round-trip verified in-process (`runtime_nexus.rs` golden). **Remaining:** (1) async completion-callback delivery (`nexus-async-completion`, requirements drafted — `DispatchCompletionCallback` is a no-op stub today); (2) end-to-end gRPC round-trip integration test (external worker poll/respond → route back cross-namespace); (3) **measure** `TestNexusApiTestSuiteWith{TemporalFailures,LegacyErrorPaths}` (40+40, never run) + `TestNexusWorkflowTestSuite` (2) |
...
| **C4a** | Nexus endpoint **admin CRUD** RPCs | ✅ | **IMPLEMENTED (reconciled 2026-06-22).** Spec `api-conformance-nexus-admin` landed (checkboxes reconciled). Live store-backed registry (`runtime/src/nexus.rs`: `NexusEndpointStore` + `InMemoryNexusEndpointStore`, server-authored UUID/version CAS/unique-name/id-paginated; `NexusEndpointRegistry` owned `resolve`), `NexusEndpointAdmin` validation+translation+error mapping (`edge/src/nexus_endpoint.rs`), 5 RPCs wired (`grpc/operator_service.rs`), 6 config knobs (`tokeira-config`), shared store in `tokeirad` so a runtime `CreateNexusEndpoint` resolves for dispatch. 13/13 edge unit tests pass. Remaining: operator-measure the 17 admin tests; author proptests P1–P4 (example coverage exists). | api-conformance-nexus-admin ✅ |
| **C4a-runtime-prereq** | live `NexusEndpointRegistry` | ✅ | Done as part of C4a — no longer a static `Arc<HashMap>`; store-backed with owned `resolve`, wired via `new_with_nexus_and_shards_and_endpoint`. | runtime/src/nexus.rs |
| **C4b** | Nexus **operation execution** / task transport | 🟡 | `PollNexusTaskQueue`, operation lifecycle, Nexus-in-workflow. **Not** in `api-conformance-nexus-admin` (explicit non-goal). **Landed (reconciled 2026-06-22):** `kernel-nexus-operations` (events/state/transitions, all tasks done), `runtime-nexus-dispatch` (publisher dispatch + endpoint resolution + timeout scanner, all tasks done), `edge-nexus-task-transport` (broker + poll/respond handlers + translation, phases 1–3 done), `runtime-nexus-http-client` (reqwest client, sync/async mapping + links + op-token, done). Plus on `main`: conflict-policy resolution, v1.62 `RespondNexusTaskFailed.failure` DTO, `worker_may_ignore` replay flag, UseExisting→success. **Remaining:** (1) **async completion-callback delivery** — `DispatchCompletionCallback` is a no-op stub, so started async ops never deliver their result back to the caller; drafted as `nexus-async-completion` (requirements only). (2) **end-to-end gRPC round-trip** — **VERIFIED 2026-06-22** by Odori's `nexus-roundtrip-probe` against a real `tokeirad` (CreateNexusEndpoint worker-target → control-ns workflow schedules → external `PollNexusTaskQueue` → `RespondNexusTaskCompleted` SyncSuccess → `"pong"` routed back cross-namespace); a permanent in-repo regression test under `apps/tokeirad/tests/` is still to be added so tokeira CI guards it. (3) **Measure** `TestNexusApiTestSuiteWith{TemporalFailures,LegacyErrorPaths}` (40+40, never run) + `TestNexusWorkflowTestSuite` (2). | edge-nexus-task-transport / kernel-nexus-operations / runtime-nexus-dispatch / runtime-nexus-http-client / nexus-async-completion |
...
C1 (in flight) → C3 (cheap re-run) → C2 (triage) → C9 (panic fix —
reclassifies 267 `unfinished` to real pass/fail) → C7 (re-triage) → C4b (async-completion + e2e
gRPC round-trip + measure the 82 op-execution tests) → C8 (classify). Done: C4a, C5a, C5b, C6.
...
- **Fix locus:** `api-conformance-nexus-admin` — **IMPLEMENTED (reconciled 2026-06-22).** The
  registry is now live/store-backed (the static `Arc<HashMap>` prereq is resolved: `resolve` returns
  an owned config), validation/translation/5 RPCs/config knobs all landed, shared store wired in
  `tokeirad`; 13/13 edge unit tests pass. Pending: operator-measure the ≈17 admin tests and author
  proptests P1–P4 (example coverage exists).
- **Category:** real-gap (tracked) — **done.**
...
- **Fix locus:** `edge-nexus-task-transport` / `kernel-nexus-operations` / `runtime-nexus-dispatch`
  / `runtime-nexus-http-client` (+ `nexus-async-completion`, `nexus-retry-policy`, `nexus-multi-cluster`).
  **Explicit non-goal of `api-conformance-nexus-admin`.** **Reconciled 2026-06-22:** the first four
  specs landed (task transport, kernel ops, dispatch, HTTP client). Remaining: async completion-callback
  delivery (`nexus-async-completion`, requirements drafted; `DispatchCompletionCallback` is a no-op
  stub), an end-to-end gRPC round-trip test, and measuring the 82 op-execution tests.
  `nexus-retry-policy` and `nexus-multi-cluster` are empty placeholder dirs (no spec authored).
