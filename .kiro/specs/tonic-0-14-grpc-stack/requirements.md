...
- **Binding Inventory:** the set of services, methods (with input type, output type, and
  ...
  the Generated Bindings' descriptor sets, excluding the `google.protobuf` package. The
  vendored tree carries no `descriptor.proto`, so that package is whichever copy of the
  descriptor language the compiler bundles; the Temporal and Tokeira packages reference it
  only for options, and it is not a surface Tokeira declares.
...
of `tonic`, `prost`, `prost-types`, `hyper-util`, and `tower-http`, every one on the
Target Stack line.
...
bindings generated by the Legacy Stack; the compiler-bundled `google.protobuf` package
MAY differ between the two compilers and is outside the inventory.
...
`grpc.reflection.v1alpha`; the `v1alpha` listing SHALL equal the Legacy Stack's listing,
and the `v1` listing SHALL equal it with the `v1` reflection service's own name in place
of the `v1alpha` one, over the same descriptors.
...
byte, with two library-owned strings excepted and recorded at their Target Stack values:
the wording of the decode-limit rejection (its `OUT_OF_RANGE` code is unchanged) and the
CORS `vary` value the public listener attaches (`origin, access-control-request-method,
access-control-request-headers` in place of `access-control-request-headers`). Reflection
additionally answers the `v1` protocol per 3.5.
