...
- [x] 0.1 (CM1) Add `reset_reapply_type`, `reset_reapply_exclude_types`, inert `post_reset_operations`
...
- [x] 0.2 (CM2) Broaden `validate_reset_target` (`workflow_service.rs:5723`) to `finish ∈ [2,
...
- [x] 1.1 (CM4) Drop `expect_open` from the reset path (`kernel.rs:1302`); ensure
      `materialize_reset_successor` (memory + DSQL + trait) loads a CLOSED base's full history.
- [x] 1.2 (CM5) Resolve base (by run_id, default current) + current (chain tip) independently at the
      edge (replace `resolve_execution_run_key` for reset); new run becomes chain current.
- [x] 1.3 (CM3) Rework the kernel/lane reset applier: author `WorkflowTaskFailed{RESET, BaseRunId,
...
- [x] 1.4 (CM5) Terminate the CURRENT run iff running (identity=resetter, force-fail started WFT,
      mark WorkflowWasReset); skip if already closed.
- [x] 1.5 (CM5) Linking: add `reset_run_id` (base→new) + `base_execution` (new→base) to execution
...
- [x] 1.9 (F11 verify) Confirm the successor re-advertises replayed-pending activities for polling
      (`RW/TestResetWorkflow` drives 2 remaining activities to completion).
- [x] 1.10 Verify Slice-1 leaves green: `WR/{NoBaseCurrentClosed,NoBaseCurrentRunning,
...
- [x] 2.1 (CM6) Add `WorkflowExecutionUpdateAdmitted` to `HistoryEventKind` (event.rs), serde-default;
      mirror in `history_serializer.rs` + DSQL codec; serde round-trip test.
- [x] 2.2 (CM6) Reapply walk `[fork+1..end]` (memory + DSQL) gated by the exclude set: SIGNALED
...
- [x] 2.3 (CM6) Schedule a WFT re-delivering each reapplied admitted update on the new run (Tier 2.12
...
- [x] 2.5 Verify: `RW/{ExcludeNoneReapplyDefault,ExcludeNoneReapplyAll,ExcludeNoneReapplySignal,
...
- [x] 3.1 (F8) Treat a signal buffered during the in-flight fork-point WFT as a post-fork
      reapply-eligible event; reapply under SIGNAL-eligible, drop under reapply-NONE.
- [x] 3.2 Verify: `RW/{TestBufferedSignalIsReappliedOnReset,TestBufferedSignalIsDroppedOnReset}`.
      `cargo test --workspace` + regress. Commit.

## Slice 4 — Reset-with-child reconnection → 28/28

- [x] 4.1 (CM8) Reapply child non-init resolution events onto the new run only when the child's
...
- [x] 4.2 Verify: `WRC/{TestResetWithChild_AfterStartingChild,TestResetWithChild_AfterChildCompletes,
