...
//! Dagger engine runs it or a CI agent does (who *invoked* it is the
...
use crate::{
    BoundProvisionerSource, BuildError, SourceSnapshot, materialize_snapshot,
...
pub async fn build_provisioner(
    ...
    // fails the build rather than silently re-resolving. Deliberately cold:
    // no cache mounts — only a pure build may be memoised by identity.
    let base = query
        .container()
        .from(&request.build_image)
        .with_exec(vec!["sh", "-c", APT_LINE])
    ...
    // and the typed exec error carries the harness output that names it.
    ...
    let test_output = base.with_exec(test_args.clone()).stdout().await?;
    tracing::debug!(
        bytes = test_output.len(),
        "hermetic closure tests passed with captured output"
    );
    ...
    for target in &request.targets {
        ...
        let builder = base
            .with_exec(vec!["rustup", "target", "add", &target.0])
            .with_exec(vec![
                "cargo",
                "build",
                "--locked",
                ...
                &target.0,
                "-p",
                crate::GENERATED_ROOT_PACKAGE,
            ])
            .with_exec(vec!["strip", &built]);
...
mod tests {
    ...
    use crate::{
        ProvisionerClosure,
        testing::{canned_artifact_bytes, canned_client},
    };
    ...
    #[tokio::test]
    async fn builds_tests_exports_and_attests_the_exported_bytes() {
        let repo = tempfile::tempdir().expect("tempdir");
        let out = tempfile::tempdir().expect("tempdir");
        let (client, wire) = canned_client().await;
        let request = request(repo.path(), out.path());

        let bundle = build_provisioner(&request, &client)
            .await
            .expect("pipeline");
        ...
        // The artifact attests the EXPORTED bytes (the canned wire writes
        // deterministic bytes for the export's host path).
        let out_path = out.path().join("aarch64-unknown-linux-gnu/tkp");
        assert_eq!(bundle.artifacts.len(), 1);
        let on_disk = std::fs::read(&out_path).expect("exported artifact exists");
        assert_eq!(
            on_disk,
            canned_artifact_bytes(&out_path.display().to_string())
        );
        assert_eq!(bundle.artifacts[0].sha256, sha256_hex(&on_disk));

        // The source fed to the engine is the MATERIALIZED snapshot dir;
        // tests ran with --locked and the closure's -p set; the cross-build
        // selects the generated member of the staged workspace. Chain
        // fragments spread across id-resolution requests, so the whole run
        // is asserted over the transcript.
        let transcript = wire.transcript();
        for fragment in [
            "/source",
            r#"["cargo","test","--locked","-p","tokeira-alpha"]"#,
            "rust:1.95-slim-bookworm@sha256:",
        ...
        assert_eq!(bundle.tests.command, "cargo test --locked -p tokeira-alpha");
    ...
    #[tokio::test]
    async fn a_floating_tag_build_image_is_refused() {
        let repo = tempfile::tempdir().expect("tempdir");
        let out = tempfile::tempdir().expect("tempdir");
        let (client, wire) = canned_client().await;
        ...
        let err = build_provisioner(&request, &client)
            .await
            .expect_err("floating tag refuses");
        assert!(
    ...
    #[tokio::test]
    async fn empty_targets_and_empty_closure_are_refused() {
        let repo = tempfile::tempdir().expect("tempdir");
        let out = tempfile::tempdir().expect("tempdir");
        ...
        assert!(build_provisioner(&no_targets, &client).await.is_err());

        let mut no_crates = base;
        no_crates.bound_source.testing_clear_crates();
        assert!(build_provisioner(&no_crates, &client).await.is_err());
    }

    #[tokio::test]
    async fn every_requested_target_is_built_and_attested() {
        let repo = tempfile::tempdir().expect("tempdir");
        let out = tempfile::tempdir().expect("tempdir");
        let (client, _wire) = canned_client().await;
        let mut request = request(repo.path(), out.path());
