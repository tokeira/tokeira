...
pub use deployment::{ConfigSource, DeploymentBindingMetadata, RecordedDefinition};
...
///
/// These are a `tkr`↔`tkp` process-spawn wire protocol, not deployment
/// data. They live here by explicit decision (deployment-repository spec):
/// both consumers sit outside this crate, and the shell crate cannot host
/// them without creating a `tkr → tokeira-tkp` lib dependency that is
