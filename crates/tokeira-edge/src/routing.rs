...
impl EdgeRouter for LocalOnlyRouter {
    async fn route_workflow(&self, _namespace: &str, _workflow_id: &str) -> EdgeResult<RouteTarget> {
        Ok(RouteTarget::Local)
    }

    async fn route_task_queue(&self, _namespace: &str, _task_queue: &str) -> EdgeResult<RouteTarget> {
        Ok(RouteTarget::Local)
    }
}
