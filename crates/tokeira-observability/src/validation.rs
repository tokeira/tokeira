//! Production validators for rendered observability artifacts.
//!
//! These validators make Alloy scrape, dashboard, and alert conventions
//! executable. They are intentionally deterministic and filesystem-only so
//! callers do not require Grafana, Prometheus, Loki, Mimir, Docker, or AWS
//! credentials.
...
/// Validation error for rendered observability artifacts.
...
/// Alloy scrape-job validator for rendered configuration.
#[derive(Debug, Default)]
pub struct AlloyConfigValidator;

impl AlloyConfigValidator {
    /// Require every named `prometheus.scrape` component in rendered Alloy
    /// configuration.
    pub fn validate_scrape_jobs(
        path: &Path,
        contents: &str,
        expected_jobs: &[&str],
        ...
        for job in expected_jobs {
            ...
            if !contents.contains(&declaration) {
                return Err(ArtifactValidationError::new(
                    path,
                    format!("prometheus.scrape[{job}]"),
                    format!("missing expected scrape job `{job}`"),
                ));
            }
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn alloy_validator_rejects_a_missing_scrape_job() {
        let error = AlloyConfigValidator::validate_scrape_jobs(
            Path::new("alloy.alloy"),
            "prometheus.scrape \"tokeirad\" {}",
            &["tokeirad", "grafana"],
        )
        .unwrap_err();

        assert_eq!(error.field, "prometheus.scrape[grafana]");
        assert!(error.to_string().contains("missing expected scrape job"));
    }
