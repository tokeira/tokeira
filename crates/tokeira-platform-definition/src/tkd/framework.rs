...
impl DefinitionFrontend for TkdFrontend {
        ...
        C: Serialize,
    {
        ...
        let (graph, config) = crate::tkd::interpret(source_text, &bridge, &(), parts)
            .map_err(|error| diagnostic(&self.format, source, &source_map, error))?;
        ...
        C: Serialize,
    {
        ...
        let evaluate_config = |source: FrontendSource<'_>,
                               parts: &dyn tokeira_platform::definition::SourceResolver,
                               label: &str|
         -> Result<crate::tkd::Value<HostValue>, Vec<String>> {
            ...
            let (_, config) = crate::tkd::interpret(text, &bridge, &(), parts)
        ...
        crate::tkd::retarget_check(current_text, current_parts, &prior_config, &current_config)
    }
}
