...
impl TypeTable {
    ...
    /// cross-document `use`.
    pub(crate) fn is_pub_type(&self, name: &str) -> bool {
    ...
    pub(crate) fn adopt(&mut self, source: &TypeTable, name: &str) -> bool {
    ...
    pub(crate) fn adopt_missing(&mut self, source: &TypeTable) {
...
impl FnTable {
    pub(crate) fn get(&self, name: &str) -> Option<&ItemFn> {
        self.fns.get(name)
    }
}
...
pub(crate) fn collect(file: &File) -> Result<(TypeTable, FnTable), EvalError> {
...
/// deep).
pub(crate) fn collect_uses(file: &File) -> Result<Vec<UseDecl>, EvalError> {
...
pub(crate) fn fn_param_names(f: &ItemFn) -> Result<Vec<String>, EvalError> {
