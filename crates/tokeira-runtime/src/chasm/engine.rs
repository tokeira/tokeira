...
impl ChasmEngine {
    ...
    pub async fn apply_side_effect_outcome(
        ...
        for _ in 0..attempts {
            ...
            // Closed lifecycle means every callback has settled. A held task on
            // such a root is anomalous; expose it rather than mutate closed state.
            // Completed activities with pending callbacks remain lifecycle Running.
            if root
                .metadata
                .lifecycle_state
    ...
    pub async fn execute_due_pure_tasks(
        ...
        for _ in 0..attempts {
            ...
            // A closed root cannot legitimately hold work. Preserve an empty
            // stale wake as a no-op, but surface a held-task lifecycle anomaly.
            if root
                .metadata
                .lifecycle_state
                .is_some_and(LifecycleState::is_closed)
...
mod tests {
    ...
    #[tokio::test]
    async fn generic_paths_reject_closed_roots_with_held_work_but_ignore_stale_delivery() {
        ...
        .await;
        let mut root = engine.root_node(&key).await.unwrap().unwrap();
        let task = root.metadata.outbox.side_effect_tasks[0].clone();
        let expected = ExpectedVersion::Vt(root.metadata.versioned_transition);
        root.metadata.lifecycle_state = Some(LifecycleState::Completed);
        assert_eq!(
            repo.persist_dirty(
                &key,
                vec![NodeWrite {
        ...
        assert!(matches!(
            engine
                .apply_side_effect_outcome(&key, task.task_type_id, task.id, outcome.clone())
                .await,
            Err(ChasmError::ExecutionClosed)
        ));
        assert!(matches!(
            engine.execute_due_pure_tasks(&key, 1).await,
            Err(ChasmError::ExecutionClosed)
        ));
        let mut stale = task.id;
        stale.offset += 100;
        assert_eq!(
            engine
                .apply_side_effect_outcome(&key, task.task_type_id, stale, outcome)
                .await
                .unwrap(),
            OutcomeApplied::NotHeld
        );
        assert_eq!(engine.root_node(&key).await.unwrap().unwrap(), root);
    }
