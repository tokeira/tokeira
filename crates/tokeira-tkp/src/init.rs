...
fn day_zero_manifest(deployment_dir: &Path) -> Result<IntegrityManifest> {
    let sidecar = deployment_dir.join(tokeira_deployment::BUNDLE_MANIFEST_BASENAME);
    if !sidecar.exists() {
    ...
    let bundle: tokeira_deployment::ProvisionerBundle =
...
/// [`EngineIdentity`](tokeira_deployment::EngineIdentity) (the closure inputs
...
mod tests {
    ...
    fn bundle_sidecar_for(bytes: &[u8]) -> tokeira_deployment::ProvisionerBundle {
        use tokeira_deployment::{
        ...
        ProvisionerBundle {
            ...
            bound: Some(tokeira_deployment::BoundProvisionerEvidence {
