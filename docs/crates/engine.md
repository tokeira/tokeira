...
`Engine::embedded()` starts a zero-listener engine with an in-memory duplex
endpoint, in-memory authoritative storage, runtime workers, and the projection
services. Optional in-memory snapshot policy restores and persists the
authoritative store across process restarts.
...
- `Engine::embedded()` and `Engine::start_with_config()` are zero-listener,
  in-memory-storage entry points.
