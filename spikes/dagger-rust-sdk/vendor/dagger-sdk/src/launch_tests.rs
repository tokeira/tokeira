...
use crate::{
    ClientConfig,
    diagnostic::{Diagnostic, DiagnosticSink, DiagnosticSinkError},
    discovery::{LaunchExecutable, NativeContextError, NativeDiscoveryInputs, NativePathSemantics},
    errors::{CliDiscoveryError, CliDiscoveryErrorKind, DiscoveryPathRole},
    launch::{
        CliLaunchProjection, CliProvisioner, CliSessionStart, CompatibilityResolver,
        ProcessSpawner, RetryClock, SessionSpawnError, SpawnErrorKind, select_compiled_cli,
        spawn_with_retry,
    },
    preflight::{ConnectionPlan, PreflightContext, ProcessInputs, preflight_with},
    provisioning_control::ProvisioningCancellation,
    provisioning_error::{ProvisionError, ProvisionErrorKind},
    target::{Architecture, ArchiveDescriptor, OperatingSystem, exact_target},
    test_support::proptest_config,
};
...
fn compatibility_warning_names_the_version_and_the_caveat() {
    ...
    .into_owned();
    assert!(warning.contains("1.0.0-beta.11.rust.1"));
