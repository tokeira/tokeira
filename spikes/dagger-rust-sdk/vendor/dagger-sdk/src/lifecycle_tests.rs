...
use crate::{
    client::{connect_with, connect_with_connector},
    config::ClientConfig,
    connection::{EngineConnection, EngineConnectionError, EngineConnectionErrorKind},
    connector::{ConnectionRequest, Connector},
    errors::{CloseError, RequestError},
...
fn lifecycle_atomic_protocol_is_exhaustive_under_loom() {
    loom::model(|| {
        use loom::{
            sync::{
                Arc as LoomArc,
                atomic::{AtomicBool as LoomBool, AtomicUsize as LoomUsize},
            },
            thread,
        };
