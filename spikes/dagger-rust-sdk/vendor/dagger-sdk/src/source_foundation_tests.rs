...
use crate::{
    config::ClientConfig,
    connection::{EngineConnection, EngineConnectionError},
    connector::{ConnectionRequest, Connector, DefaultConnector},
    discovery::{
    ...
    graphql::{RawRequest, RawResponse, ResponseData},
    preflight::{CliSourcePlan, ConnectionPlan, PreflightContext, ProcessInputs, preflight_with},
    session::{ExistingSessionInput, validate_existing_session},
    target::{
        Architecture, ArchiveDescriptor, ArchiveFormat, OperatingSystem, exact_target,
        exact_target_from_parts,
    },
    target_generated::{TARGET_CLI_VERSION, TARGET_ENGINE_VERSION, TARGET_REVISION},
    test_support::{TransportEventLog, proptest_config},
};
...
proptest! {
    ...
    fn property_06_platform_descriptors_exact_side_effect_free(
        ...
        let (engine, cli, revision) = match drift_case {
            ...
            1 => ("1.0.0-beta.11.rust.3", TARGET_CLI_VERSION, TARGET_REVISION),
        ...
        match (os, arch) {
            (Ok(os), Ok(arch)) => {
                ...
                let expected_name = format!("dagger_v1.0.0-beta.11.rust.3_{os_name}_{arch_name}.{extension}");
...
fn six_release_descriptors_match_the_published_naming_policy() {
    ...
    for (os, arch, os_name, arch_name, format, member) in cases {
        ...
        let archive = format!("dagger_v1.0.0-beta.11.rust.3_{os_name}_{arch_name}.{extension}");
...
fn explicit_discovery_expands_home_resolves_symlinks_and_ignores_irrelevant_cwd() {
    ...
    let resolved = native_fixture_path(
        "/opt/dagger-v1.0.0-beta.11.rust.3",
        r"C:\tools\dagger-v1.0.0-beta.11.rust.3.exe",
    );
